...
页面默认进入「资金图」，展示用户选中的重点大类。默认最多同时展示 32 个大类；解除限制后可以选择更多，并支持全选。可选大类由本地代理从东方财富行业/概念/地域等公开数据中归一化生成，并通过 `/api/fund-flow/focus-groups` 提供给前端下拉框。
...
- 自动回溯：用户设置回放秒数后，从 09:15 集合竞价平滑播放到当前时间，最小 10 秒。
- 大类管理：下拉展示全部大类，支持模糊搜索、选择/取消、最多 32 个限制、解除限制和全选。
- ETF 分类成交额：顶部汇总展示 ETF 总成交额，以及股票、债券、货币、商品、QDII 分项。
- 资金暗盘：顶部可在盘中曲线和资金暗盘之间切换；暗盘使用行业到个股的矩形热力图，面积按成交额、颜色按主力净流入强度，悬停查看代码和资金拆分。盘中为预览，收盘后显示收盘快照。
- Canvas 录制：只录制资金图 canvas，画布内置背景；优先输出 MP4/H.264，浏览器不支持时回退 WebM；大量大类时自动降低录制帧率并缓存单帧绘图数据，减少卡顿。
- 移动端适配：小屏用更紧凑的图表布局和小字号线尾标签；资金暗盘先展示行业总览，点击行业后查看对应个股。
...
资金暗盘首版从主力净流入和净流出两个方向提取异动样本，页面会显示样本数与市场股票总数。它是收盘资金流快照与复盘视图，不把样本汇总伪装成全市场总额。
...
- 修改页面交互和绘图时，主要改 `src/components/market` 与 `src/hooks`。
- 修改桌面和移动端样式时，主要改组件内的 Tailwind 类和 `src/app/globals.css`。
- 修改后先运行 `npm run check`，确认 TypeScript 与服务脚本正常。
